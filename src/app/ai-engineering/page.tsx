...
interface AiPayload {
  ...
  groups: (HubGroupView & { minutesDone: number; minutesTotal: number })[];
  excludedOptional: number;
}

const COMPETENCY_ORDER = ["Understand", "Build", "Evaluate", "Interview-ready"];
...
export default function AiEngineeringPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoadError(null);
      const res = await fetch("/api/tracks?track=ai");
      if (!res.ok) throw new Error("AI Engineering data failed to load");
  ...
  const stageIdx = Math.max(0, ...data.groups.map((g) => COMPETENCY_ORDER.indexOf(g.stage)));
  const competency = COMPETENCY_ORDER[Math.min(3, stageIdx)] ?? "Understand";

  return (
    <div className="mx-auto max-w-3xl space-y-10 pb-16 md:space-y-12">
      {/* hero: overall progress */}
      <header className="pt-4 md:pt-8">
        <p className="type-label flex items-center gap-2 !text-indigo-300">
          <Brain className="h-4 w-4" aria-hidden /> AI Engineering
        </p>
        ...
        <p className="type-body mt-1 text-[15px]">
          ...
          {data.excludedOptional > 0 ? ` · ${data.excludedOptional} optional excluded from pace` : ""}
        </p>
        <div className="mt-4 max-w-md">
          <ProgressBar value={data.progress.percent} label="AI Engineering progress" color="bg-gradient-to-r from-indigo-500 to-cyan-400" heightClass="h-2" />
        </div>
        
        {/* Technical Competency Stage Pipeline */}
        <div className="mt-5 rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-3.5">
          <p className="type-label !text-indigo-300 mb-2">Competency Pipeline</p>
          <div className="flex flex-wrap items-center gap-2 text-xs font-mono font-bold">
            {COMPETENCY_ORDER.map((stage, idx) => {
                ...
                <div
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all ${
                    isDone
                      ...
                      : isCurrent
                      ? "border-indigo-500/40 bg-indigo-500/20 text-indigo-200 ring-1 ring-indigo-500/30"
                      : "border-border/60 bg-border/20 text-gray-500"
                  }`}
                >
      ...
      <PlainSection title="Curriculum groups">
        <div className="space-y-2.5">
          {data.groups.map((g) => (
            <GroupCard key={g.key} group={g} defaultOpen={g.key === "foundations"} />
          ))}
        </div>
      </PlainSection>

      {/* cross-track: retrieval ↔ production */}
      <PlainSection title="Connected tracks">
        <CrossTrackLinks filter={(label) => /retrieval|production/i.test(label)} />
        <Link href="/projects" className="mt-3 inline-flex items-center gap-1 text-sm font-bold text-accent hover:underline">
          Build evidence lives in Projects <ChevronRight className="h-4 w-4" />
        </Link>
      </PlainSection>
    </div>
  );
